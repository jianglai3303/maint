import "../styles/discover.less"
import React from "react"
export default class Discover extends React.Component {
    render(){
        return (
            <div className="discover-wapper">
                <div className="top-part">
                <h3>DERNIERS PROJETS</h3>
                <h2>Nos Réalisations</h2>
                <p>Notre expertise nous permet d’intervenir sur de nombreuses typologie</p>
                </div>
                <ul className="ds-list">
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    <li className="ds-list-item">
                        <h4>DESIGND’INTERFACES</h4>
                        <div className="icon-with-line">
                            <svg width="512" height="512" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0 245.156L174.349 169.453V205.688L31.3937 265.497L174.349 325.709V362.079L0 286.241V245.156ZM309.221 66H349.103L220.025 446H180.143L309.221 66ZM337.651 323.824L480.606 264.015L337.651 203.802V167.567L512 243.136V284.221L337.651 359.924V323.824Z"></path>
                            </svg>
                        </div>
                        <p>
                            Nous créons des interfaces innovantes, esthétiques, originales et ergonomiques, des wireframes jusqu’au layout final.
                        </p>
                    </li>
                    
                </ul>
            </div>
        )
    }
};